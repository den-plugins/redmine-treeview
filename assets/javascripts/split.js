  ...
  if(jQuery("#transferred_" + id).length == 0){
    ...
    while(hasParent){
      ...
      if(jQuery("#transferred_" + parentId).length == 0){
        ...
        jQuery("#transferred_" + parentId).append("<td class='small'><img width='15' src='/images/arrow_to.png' alt='Arrow_to' onclick=\"undo_transfer(\'" + type + "\'" + ", " + "\'" + parentId + "\')\"></td>")
...
function undo_transfer(type, id){
  ...
  while(hasParent){
      ...
      var table_rows = table2.find("tbody tr");
...
function toggleNewOpts(target) {
  j("#split-new-issue-options").find("input, textarea, select").attr("disabled", !!j(target).val());
}