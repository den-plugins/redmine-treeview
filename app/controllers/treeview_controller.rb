class TreeviewController < IssuesController
    ...
    if @query.valid?
      ...
      (offset ... (offset + limit)).each do |i|
        break if @filtered_issues[i].nil?
          ...
          if params[:sort].nil?
            @issues.each do |is|
              is.item.position = 0 if is.item.position.nil?
            end
            @issues = @issues.sort_by{|p| p.item.position }
          end
  ...
  def show
    @total_journals = @issue.journals.count("journals.id", :include => [:user, :details],
    ...
    @journals = @issue.journals.find(:all, :include => [:user, :details], :limit => 10, :offset => offset,
                                     :conditions => ["#{JournalDetail.table_name}.property <> 'timelog' OR #{Journal.table_name}.notes is not null AND #{Journal.table_name}.notes <> ''"],
  ...
  def carry_over
    ...
    support_tracker_id = Tracker.find_by_name("Support").id
    @subtasks = @issue.children.select {|c| c.tracker_id != support_tracker_id}
  ...
  def carry_over_operation
    if CarryOverHelper.carry_over(params)
      ...
      render :js => "jQuery('#facebox .close').trigger('click'); jQuery(#{success_message}).insertBefore('#query_form'); jQuery(window).scrollTop(0,0);"
    else
      fail_message = "\"<div id='notif' class='flash error'>Issues have not been carried over.</div>\""
      render :js => "jQuery('#facebox .close').trigger('click'); jQuery(#{fail_message}).insertBefore('#query_form'); jQuery(window).scrollTop(0,0);"
    end
  end
